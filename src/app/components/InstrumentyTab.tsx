...
// Компонент вкладки "Инструменты"
export function InstrumentyTab({ scrollOffset, setScrollOffset }: { scrollOffset: number; setScrollOffset: (value: number) => void }) {
...
// Колонка 2 инструментов
function InstrumentyColumn2() {
  return (
    <>
      <Column title="Для монтажа ВЧ соединителей" items={[]} />
    </>
  );
}

// Колонка 3 инструментов
function InstrumentyColumn3() {
  return (
    <>
      <Column title="Моментные ключи" items={[]} />
      <Column title="Калибровочные наборы" items={[]} />
    </>
  );
}
